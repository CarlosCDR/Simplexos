...
class Simplexos extends MY_Controller
{
    /**
     * author: Ramon Silva
    ...
    {
        parent::__construct();
        $this->load->model('Simplexos_model');
    }

    public function index()
    {
        $this->data['ordens'] = $this->Simplexos_model->getOsAbertas();
        $this->data['ordens1'] = $this->Simplexos_model->getOsAguardandoPecas();
        $this->data['ordens_andamento'] = $this->Simplexos_model->getOsAndamento();
        $this->data['produtos'] = $this->Simplexos_model->getProdutosMinimo();
        $this->data['os'] = $this->Simplexos_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->Simplexos_model->getEstatisticasFinanceiro();
        $this->data['financeiro_mes_dia'] = $this->Simplexos_model->getEstatisticasFinanceiroDia($this->input->get('year'));
        $this->data['financeiro_mes'] = $this->Simplexos_model->getEstatisticasFinanceiroMes($this->input->get('year'));
        $this->data['financeiro_mesinadipl'] = $this->Simplexos_model->getEstatisticasFinanceiroMesInadimplencia($this->input->get('year'));
        $this->data['menuPainel'] = 'Painel';
        $this->data['view'] = 'Simplexos/painel';
        return $this->layout();
    }

    public function minhaConta()
    {
        $this->data['usuario'] = $this->Simplexos_model->getById($this->session->userdata('id_admin'));
        $this->data['view'] = 'Simplexos/minhaConta';
        return $this->layout();
    }

    public function alterarSenha()
    {
        $current_user = $this->Simplexos_model->getById($this->session->userdata('id_admin'));

        if (!$current_user) {
            $this->session->set_flashdata('error', 'Ocorreu um erro ao pesquisar usuário!');
            redirect(site_url('Simplexos/minhaConta'));
        }
        ...
        $result = $this->Simplexos_model->alterarSenha($senha);
    ...
    public function pesquisar()
    {
        ...
        $data['results'] = $this->Simplexos_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        $this->data['servicos'] = $data['results']['servicos'];
        $this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'Simplexos/pesquisa';
        return $this->layout();
    }
    ...
    public function emitente()
    {
        ...
        $this->data['dados'] = $this->Simplexos_model->getEmitente();
        $this->data['view'] = 'Simplexos/emitente';
        return $this->layout();
    }
    ...
    public function cadastrarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(site_url('Simplexos/emitente'));
        } else {
            ...
            $retorno = $this->Simplexos_model->addEmitente($nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo);
            if ($retorno) {
    ...
    public function editarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->Simplexos_model->editEmitente($id, $nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email);
            if ($retorno) {
    ...
    public function editarLogo()
    {
        ...
        $retorno = $this->Simplexos_model->editLogo($id, $logo);
        if ($retorno) {
            $this->session->set_flashdata('success', 'As informações foram alteradas com sucesso.');
    ...
    public function uploadUserImage()
    {
        ...
        $usuario = $this->Simplexos_model->getById($id);
        ...
        $imageUserPath = $image;
        $retorno = $this->Simplexos_model->editImageUser($id, $imageUserPath);
    ...
    public function emails()
    {
        ...
        $this->data['configuration']['base_url'] = site_url('Simplexos/emails/');
    ...
    public function excluirEmail()
    {
        ...
        if ($id == null) {
            ...
            redirect(site_url('Simplexos/emails/'));
        }
    ...
    public function atualizarBanco()
    {
        ...
        return redirect(site_url('Simplexos/configurar'));
    }

    public function atualizarSimplexos()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cSistema')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar o sistema');
            redirect(base_url());
        ...
        if (!$this->github_updater->has_update()) {
            $this->session->set_flashdata('success', 'Seu Simplexos já está atualizado!');
        ...
        if ($success) {
            $this->session->set_flashdata('success', 'Simplexos atualizado com sucesso!');
        } else {
            $this->session->set_flashdata('error', 'Erro ao atualizar Simplexos!');
        }
    ...
    public function calendario()
    {
        ...
        $allOs = $this->Simplexos_model->calendario(
            $start,
            $end,
            $status
        );
